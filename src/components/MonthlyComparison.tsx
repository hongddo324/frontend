...
export function MonthlyComparison({ onBack, selectedDate }: MonthlyComparisonProps) {
  const selectedMonthLabel = selectedDate ? `${selectedDate.getFullYear()}년 ${selectedDate.getMonth() + 1}월` : '현재';
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <Card className="p-3">
            <div className="text-center">
              ...
              <div className="text-sm font-semibold">₩{formatKoreanCurrency(totalCurrent)}</div>
          ...
          <Card className="p-3">
            <div className="text-center">
              ...
              <div className="text-sm font-semibold text-green-600">₩{formatKoreanCurrency(totalPredicted)}</div>
              <div className="text-xs text-green-600">
                -₩{formatKoreanCurrency(totalCurrent - totalPredicted)} 절약
              </div>
            </div>
          </Card>
        </div>
          ...
          <CardContent className="space-y-3">
                ...
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="font-semibold text-sm">
                      ₩{formatKoreanCurrency(card.amount)}
                    </div>
                  </div>
            ...
            <div className="pt-2 border-t mt-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">₩{formatKoreanCurrency(1184000)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                <Card key={index} className="p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="text-center">
                      ...
                      <p className="text-sm font-semibold">₩{formatKoreanCurrency(item.current)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs text-muted-foreground">9월 예상</p>
                      <p className="text-sm font-semibold text-green-600">₩{formatKoreanCurrency(item.predicted)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs text-muted-foreground">예산</p>
                      <p className="text-sm font-semibold">₩{formatKoreanCurrency(item.budget)}</p>
                    </div>
                  </div>
            ...
            <CardContent className="p-0">
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={dailySpending}>
                  ...
                  <XAxis dataKey="day" />
                  <YAxis tickFormatter={(value) => `₩${formatKoreanCurrency(value)}`} />
                  <Tooltip
